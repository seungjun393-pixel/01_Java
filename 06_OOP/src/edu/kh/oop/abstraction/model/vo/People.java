...
public class People { // 국민(사람) 클래스
	...
	public String getName() {
		return name; // name의 값을 되돌려준다. 호출한쪽(p1.getName()으로
		// name 이라는 반환값 있음
		// 반환형 : 반환되는 값의 자료형
		// return : 반환, 되돌려주다
	...
	// getter() : 반환형이 무조건 있음
	//getter는 필드에 작성된 변수값을 호출한 쪽으로 되돌려 주는것
	// -> 변수값은 자료형이 있음, 그 변수를 되돌려준다면 당연히 반환형도 존재함
	
	// setter() : 반환형이 없음
	//setter는 전달인자에 작성된 값을 통해 
	// 매개변수로 들어온 값을 객체에 필드에 세팅하는 역할
	// -> 매개변수가 항상있음(전달인자 값)
	// -> 전달인자에는 자료형이 있음 -> 매개변수 자료형이 있음
	// -> 매개변수 작성법은 : (자료형 매개변수명)
	// -> 단순히 전달값으로 필드에 값을 세팅하는 역할
	// -> 호출한 쪽으로 되돌려줄 값은 없음
	// -> 반환형도 없음(void)
	
	// alt + s + r 또는 상단메뉴 Source
