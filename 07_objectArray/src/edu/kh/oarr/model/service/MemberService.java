...
public class MemberService {
	...
	public MemberService() { // 기본 생성자
		// memberArr 배열 0,1,2 인덱스 초기화
         ...
         memberArr[1] = new Member("user02","pass02","김길동",25,"부산");
         memberArr[2] = new Member("user03","pass03","최길동",35,"진주");
	...
	public void displayMenu() {
		...
		do {
			...
			break; 
			default : System.out.println("잘못입력하셨습니다. 메뉴에잇는 번호만 입력하세요.");
			}
			
		}while(menuNum != 0);
	}
	
	// 사이드 메서드 : 메인기능은 아니나, 다른메서드에 도움이 되는 메서드
    // memberArr의 비어있는 인덱스번호를 반환하는 역할
	...
	public int emptyIndex() {
		...
		for(int i = 0; i<memberArr.length; i++) {
			...
			if(memberArr[i] == null){
				return i;
				// 현재 메서드를 종료하고, 호출한곳으로 i값가지고 되돌아감
			}
			
		}
		
		// for 문을 수행했지만, 인덱스값이 return되지 않은경우
		// 해당위치 코드가 수행된다
		// -> for 문에서 return 되지않았다 == 배열에 빈칸이 없다
		return -1;
	}
	...
	public String signUp() {
		...
		// -> 빈자리가 있을때 : 빈 공간의 인덱스 번호 얻어오기
		// -> 없을때 : -1 반환
		int index = emptyIndex();
		
		if(index == -1) { // 비어있는 공간없음 -> 회원가입 불가
	...
	public String login() {
		...
		// 2-1) 현재 접근한 요소가 null 이 아닌지 확인
			if(memberArr[i] != null) { // 회원 정보가 있을 경우
				...
				// loginMember에 현제 접근중인 memberArr[i] 요소에
				// 저장된 주소를 얕은 복사
					loginMember = memberArr[i];
					break;
					// 더이상 같은 아이디,비밀번호 없을거라 예상
					// -> 효율을 위해 for 문 종료
		...
		// 3) 로그인 성공/실패 여부에 따라 결과값 반환
		// -> 로그인 성공 : 홍길동님 환영합니다!
		// -> 로그인 실패 : 아이디 또든 비밀번호가 일치하지 않습니다
		if(loginMember == null) {
	...
	public String selectMember() {
		...
		for(int i = 0; i < memberArr.length; i++) {
			...
			if(loginMember.getMemberId().equals(memberArr[i].getMemberId())) {
				
				return "이름 : "+memberArr[i].getMemberName()
						+"\n아이디 : "+memberArr[i].getMemberId()
						+"\n나이 : "+memberArr[i].getMemberAge()
						+"\n지역 : "+memberArr[i].getRegion();
